...
PERFORMANCE:
    - Computation: <0.02 ms per 20 ms control cycle
...
def create_handover_profile(N: int, dt: float = DT, 
    ...
    base_jitter = rng.normal(0, 15.0, N)  # 15ms std dev
...
def update_bbr_leo_aware(rtt_ms: float, bytes_acked: float, state: CCState, 
    ...
    if state.phase == "STARTUP":
        ...
        state.pacing_gain = 1.0
        target_rate = state.bw_est * 0.75
        state.rate_kbps = 0.3 * target_rate + 0.7 * state.rate_kbps
        
        if state.probe_rtt_done_stamp is None:
            state.probe_rtt_done_stamp = step
        
        if step - state.probe_rtt_done_stamp >= 20:
            state.phase = "PROBE_BW"
            state.cycle_index = 0
            state.cycle_stamp = step
            state.probe_rtt_done_stamp = None
            state.rate_kbps = state.prior_cwnd
            state.rtprop_stamp = step
...
def run_bootstrap_analysis(results: Dict, outdir: str, tag: str, 
    ...
    for baseline in baselines:
        ...
        for config in compare_configs:
            ...
            _, base_metrics = results[base_key]
            base_goodput_series = np.array(base_metrics.sending_rates)
            config_goodput_series = np.array(config_metrics.sending_rates)
...
def plot_bootstrap_results(bootstrap_df: pd.DataFrame, outdir: str, tag: str):
    ...
    for idx, (metric, label) in enumerate(zip(metrics, metric_labels)):
        ...
        ax.errorbar(deltas, y_pos, 
                   xerr=[deltas - ci_lows, ci_highs - deltas],
...
def simulate_plc(N: int, seed: int, baseline: str,
    ...
    discrepancy = abs(metrics.packets_sent - (metrics.packets_received + metrics.packets_lost))
    max_allowed_discrepancy = max(10, int(N * 0.01))  # 1% threshold
    
    if discrepancy > max_allowed_discrepancy:
        error_msg = (f"INTEGRITY ERROR: Packet accounting mismatch!\n"
                    ...
                    f"  This may indicate a bug in packet counting logic.")
        
        # ✅ FIX J: Strict accounting mode
...
Examples:
  ...
  python %(prog)s --rtt-rule --rtt-spike-threshold 1.5
  ...
  python %(prog)s --rtt-rule --outdir results_rtt
    ...
    ap.add_argument("--g-boost", type=float, default=1.5,
                    help="PRE gain multiplier when handover imminent (default: 1.5)")
    ...
    ap.add_argument("--rtt-spike-threshold", type=float, default=1.5,
                    help="RTT spike threshold (short/long ratio) for HO detection (default: 1.5)")
